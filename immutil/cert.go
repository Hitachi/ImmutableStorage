...
func CreateSelfKeyPair(subj *pkix.Name, caKeyDir string) (caPrivFile, caCertFile string, retErr error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			privPem, pubPem, skiStr, err2 := GenerateKeyPair(subj)
			if err2 != nil {
				retErr = err2
				return
			}
...
func GenerateKeyPair(subj *pkix.Name) (privPem, pubPem []byte, skiStr string, retErr error) {
	...
	if err != nil {
		retErr = fmt.Errorf("Failed to create a certificate: %s\n", err)
		return
	}
...
//	fmt.Printf("log: cert: commonname=%s\n", cert.Subject.CommonName)
	return
}
